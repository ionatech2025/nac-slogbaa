...
export async function getLiveSessions(token) {
  assertToken(token)
  const res = await apiClient(token).get('/api/live-sessions')
...
export async function getAdminLiveSessions(token) {
  assertToken(token)
  const res = await apiClient(token).get('/api/admin/live-sessions')
  return parseListResponse(res)
}

export async function createLiveSession(token, data) {
  assertToken(token)
  const res = await apiClient(token).post('/api/admin/live-sessions', data)
  return parseResponse(res)
}

export async function updateLiveSession(token, id, data) {
  assertToken(token)
  const res = await apiClient(token).put(`/api/admin/live-sessions/${id}`, data)
  return parseResponse(res)
}

export async function deleteLiveSession(token, id) {
  assertToken(token)
  const res = await apiClient(token).delete(`/api/admin/live-sessions/${id}`)
  return parseResponse(res)
}
...
export async function registerForLiveSession(token, sessionId) {
  assertToken(token)
  const res = await apiClient(token).post(`/api/live-sessions/${sessionId}/register`)
  return parseResponse(res)
}
...
export async function unregisterFromLiveSession(token, sessionId) {
  assertToken(token)
  const res = await apiClient(token).delete(`/api/live-sessions/${sessionId}/register`)
  return parseResponse(res)
}
